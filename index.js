...
function setListeners(userCard) {
    ...
    deleteBtn.addEventListener('click', (event) => {
        ...
        const selUser = document.querySelector('.users')
        const newLocalStorage = storage;
        for (let i = 0; i < allUser.length; i++) {
            if (userEmail === allUser[i].dataset.email) {
                selUser.removeChild(allUser[i])
                delete newLocalStorage[allUser[i].dataset.email]
                localStorage.users = JSON.stringify(newLocalStorage)
            }
        }
    ...
    changeBtn.addEventListener('click', (event) => {
        ...
        const newTel = document.querySelector('#phone')
        const allUser = document.querySelectorAll('.user')
        for (let i = 0; i < allUser.length; i++) {
            if (changeEmail === allUser[i].dataset.email) {
                const { name, secondName, email, tel } = storage[allUser[i].dataset.email]
                newName.value = name
 ...
 */
function createCard({ name, secondName, email, tel }) {
    return `
        <div data-user=${email} class="user-outer">
            ...
            <div class="menu">
                <button data-delete-user-email=${email} class="delete btn btn-outline-danger">Удалить</button>
                <button data-change-user-email=${email} class="change btn btn-outline-success">Изменить</button>
            </div>
        </div>
    `
}
...
function addCard(e) {
    ...
    if (!newEmail.value
        ...
        resetInputs(newName, newSecondName, newEmail, newTel)
        return
    }
    ...
    if (newEmail.value in storage) {
        ...
        const allUser = document.querySelectorAll('.user')
        const allInfo = document.querySelectorAll('.user-info')
        for (let i = 0; i < allUser.length; i++) {
            if (newEmail.value === allUser[i].dataset.email) {
                allInfo[i].children[0].innerText = newName.value;
                allInfo[i].children[1].innerText = newSecondName.value;
                allInfo[i].children[2].innerText = newEmail.value;
                allInfo[i].children[3].innerText = newTel.value;
            }
        }
    } else {
        storage[newEmail.value] = data
